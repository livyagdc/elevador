...
const int portaElevador = 6;
const int chamarElevador = 5;
...
{
    motor.attach(7);
    pinMode(portaElevador, OUTPUT);
    pinMode(chamarElevador, INPUT_PULLUP);
    Serial.begin(9600);
    ...
    digitalWrite(portaElevador, LOW);
}

// ================= LOOP PRINCIPAL =================
void loop()
{
    // --- Detectar botão de chamada ---
    if (digitalRead(chamarElevador) == LOW)
    {
        Serial.println("Digite o andar desejado (0-9):");
        
        // Espera a entrada serial (sem delay, mas bloqueia brevemente a leitura do botão)
    ...
    switch (estadoElevador)
    {
        ...
        case SUBINDO: 
            ...
            if (tempoInicioMovimento == 0) {
                motor.write(180); // Ligar motor para subir
        ...
        case DESCENDO: 
            // Passo 1: Ligar o motor e registrar o tempo
            if (tempoInicioMovimento == 0) {
                motor.write(0); // Ligar motor para descer
                tempoInicioMovimento = agora;
            }
...
{
    if (!portaAberta) {
        digitalWrite(portaElevador, HIGH);
        portaAberta = true;
